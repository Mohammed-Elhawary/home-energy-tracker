...
public class InsightServiceRoute {
    ...
    public RouterFunction<ServerResponse> insightRoutes() {
        return route("insight-service").route(RequestPredicates.path("/api/v1/insight/**"), http())
                .before(uri("http://localhost:8085")).filter(CircuitBreakerFilterFunctions
                        .circuitBreaker("InsightServiceCircutBreaker", URI.create("forward:/fallback/insight-service")))
  .build();
    }
